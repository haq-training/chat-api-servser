...
const userResolver: IResolvers = {
    ...
    Query: {
        ...
        listFriend :async (_parent,{},context)=>{
            ...
            const friendships_follow = await db.Friendship.findAll({
                where: {
                    [Op.or]: [
                        { RequesterId: user.id },
                        { AddresseeId: user.id },
                ...
                attributes: [
                    [sequelize.literal('CASE WHEN `RequesterId` = :userId THEN `AddresseeId` ELSE `RequesterId` END'), 'friend_follow_Id'],
                ],
                raw: true,
                replacements: { userId: user.id },
            });
            ...
            const friend_follow_Ids = friendships_follow.map(element => (element as unknown as { friend_follow_Id: number }).friend_follow_Id);
            const uniqueFriend_follow_Ids = [...new Set(friend_follow_Ids)];
            interface User {
                id: number;
                email: string
                firstName : string
            ...
            const promises_follow = uniqueFriend_follow_Ids.map(async(element)=> {
                const userInfo = await db.users.findByPk(element);
                const userData = await fetchUserData(userInfo);
                return userData;
            });
            try {
                userInfos.push(...await Promise.all(promises));
                user_block_Infos.push(...await Promise.all(promises_block));
                user_follow_infos.push(...await Promise.all(promises_follow));
            } catch (error) {
                console.error( error);
            }
    ...
    Subscription: {
        ...
        friendRequestReceived: {
            subscribe: (parent, { userId }, { pubsub }) => pubsub.asyncIterator(`FRIEND_REQUEST_RECEIVED_${userId}`)
        },
    },
